...
def add_tracking(message, tgbot):
    ...
    if user['tracking'].get(track_name):
        ...
        user['tracking'][track_name] = {"url":track_url, "name":track_name, "update":time.time()}
        db.save(
            message.from_user.id,
           {"$set":user},
        )
        tgbot.send_photo(message.chat.id, caption='😎 Готово, теперь я слежу за этой страницей', photo=open(response['path']+'/img.png', 'rb'))
